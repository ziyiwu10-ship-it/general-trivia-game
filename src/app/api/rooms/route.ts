...
export async function POST(req: NextRequest) {
  try {
    const { hostName, category, numQuestions } = await req.json();
    ...
    const { data: player, error: playerError } = await supabase
      .from("players")
      .insert({ room_id: room.id, name: hostName.trim().slice(0, 20), is_host: true })
      .select()
      .single();
    ...
    return NextResponse.json({
      ...
      player: { id: player.id, name: player.name, token: player.client_token, isHost: true },
    });
  } catch (err) {
